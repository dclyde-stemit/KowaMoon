...
export interface IStorage {
  ...
  getLatestAnime(limit?: number): Promise<Anime[]>;
  recordView(animeId: string): Promise<void>;
}

export class MemStorage implements IStorage {
  private animes: Map<string, Anime>;
  private viewCounts: Map<string, number>;
  private lastViewTime: Map<string, number>;
  ...
  async getTrendingAnime(limit: number = 10): Promise<Anime[]> {
    const animeList = Array.from(this.animes.values());
    const oneDayAgo = Date.now() - (24 * 60 * 60 * 1000);
    
    return animeList
      .map(anime => ({
        anime,
        viewCount: this.viewCounts.get(anime.id) || 0,
        lastView: this.lastViewTime.get(anime.id) || 0,
      }))
      .filter(item => item.lastView > oneDayAgo)
      .sort((a, b) => b.viewCount - a.viewCount)
      .slice(0, limit)
      .map(item => item.anime);
  }

  async getLatestAnime(limit: number = 10): Promise<Anime[]> {
    return Array.from(this.animes.values())
  ...
  async recordView(animeId: string): Promise<void> {
    const currentCount = this.viewCounts.get(animeId) || 0;
    this.viewCounts.set(animeId, currentCount + 1);
    this.lastViewTime.set(animeId, Date.now());
  }
}
